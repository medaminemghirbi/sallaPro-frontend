...
@Component({
  selector: 'app-anyalze-per-consultation',
  templateUrl: './anyalze-per-consultation.component.html',
  styleUrls: ['./anyalze-per-consultation.component.css']
})
export class AnyalzePerConsultationComponent implements OnInit {
  currentuser: any;
  ...
  updateimage(f: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.doctorService.analyzeImageConsultation(imageformadata, this.currentuser.id, this.consultationId ).subscribe(
          (response) => {
